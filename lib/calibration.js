/**
 * Calibración quincenal automática del modelo de predicciones.
 *
 ...
 * Supabase. Cada 14 días se recalcula el factor de corrección de cada
 * mercado comparando la tasa de acierto real contra la probabilidad que
 * predijimos, y el contador se reinicia para la siguiente ventana.
 *
...
const CALIBRATION_CYCLE_DAYS = 14;
...
export async function runDailyCalibrationTick() {
  ...
  const lastCalibratedAt = state?.last_calibrated_at ? new Date(state.last_calibrated_at).getTime() : null;
  const dueForCalibration = !lastCalibratedAt
    || (Date.now() - lastCalibratedAt) >= CALIBRATION_CYCLE_DAYS * 24 * 60 * 60 * 1000;
