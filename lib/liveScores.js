...
async function enrichFromSummary(entry, sportSlug = DEFAULT_SLUG) {
  ...
  try {
    const res = await fetch(`${summaryUrl(sportSlug)}?event=${entry.espnEventId}`, { headers: FETCH_HEADERS, signal: AbortSignal.timeout(10_000) });
    ...
    const officialsRaw = summary.gameInfo?.officials;
    const officials = Array.isArray(officialsRaw) ? officialsRaw : officialsRaw ? [officialsRaw] : [];
    const mainOfficial = officials.find((o) => o.position?.name === 'Referee' || o.position?.id === '1' || o.order === 1) || officials[0];
    const refereeName = mainOfficial?.fullName || mainOfficial?.displayName || entry.refereeName || null;
...
async function fetchEspnLiveMap(dateIso, sportSlug = DEFAULT_SLUG, { includeScheduled = false } = {}) {
  ...
  // Timeout defensivo: sin esto, una sola liga con ESPN lento puede estancar
  // todo un Promise.all de decenas de competiciones (p. ej. Inicio) hasta el
  // límite de la función serverless.
  const res = await fetch(`${scoreboardUrl(sportSlug)}?dates=${espnDate}`, { headers: FETCH_HEADERS, signal: AbortSignal.timeout(10_000) });
...
async function fetchFallbackScores(dateIso) {
  const map = new Map();
  try {
    const res = await fetch(FALLBACK_GAMES_URL, { headers: FETCH_HEADERS, signal: AbortSignal.timeout(10_000) });
