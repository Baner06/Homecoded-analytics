...
function espnToFixtureName(displayName) {
...
export function liveKey(dateIso, homeName, awayName) {
  ...
  return `${dateIso}|${home.displayName}|${away.displayName}`;
}
...
function parseDetails(details, homeTeamId, awayTeamId) {
  ...
  for (const item of details || []) {
    ...
    if (text === 'Goal' && !item.shootout && minute <= 45) {
...
async function enrichFromSummary(entry) {
  if (!entry?.espnEventId) return entry;

  try {
    const res = await fetch(`${ESPN_SUMMARY}?event=${entry.espnEventId}`, { headers: FETCH_HEADERS });
...
async function fetchEspnLiveMap(dateIso) {
  ...
  for (const event of data.events || []) {
    ...
    const key = liveKey(
      dateIso,
      espnToFixtureName(home.team?.displayName),
      espnToFixtureName(away.team?.displayName)
    );

    pending.push(
      enrichFromSummary(parsed).then((enriched) => {
  ...
  try {
    ...
    for (const game of data.games || []) {
      const active = game.time_elapsed === 'live'
        || (String(game.finished).toUpperCase() === 'FALSE' && game.time_elapsed !== 'notstarted');
      if (!active) continue;
...
export function getLiveResultForMatch(liveMap, dateIso, homeTeam, awayTeam) {
  return liveMap.get(liveKey(dateIso, homeTeam, awayTeam)) ?? null;
}
