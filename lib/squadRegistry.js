...
const SQUAD_SYNC_TTL = 30 * 60 * 1000;
...
export async function syncSquadRegistry({ fixtures = [], espnByPair = new Map(), force = false } = {}) {
  const now = Date.now();
  if (!force && registrySyncedAt && now - registrySyncedAt < SQUAD_SYNC_TTL && wcTeamStats.size > 0) {
    return;
  }
  ...
  for (const f of finished) {
    ...
    eventIds.push(String(eventId));
  }

  for (let i = 0; i < eventIds.length; i += SYNC_BATCH_SIZE) {
    const batch = eventIds.slice(i, i + SYNC_BATCH_SIZE);
    await Promise.all(batch.map((eventId) => syncMatchFromSummary(eventId)));
  }
