...
import {
  loadSofaRefereesForCompetitionDate,
  invalidateSofaCompetitionRefereeCache,
  getSofaRefereeForMatch,
} from './sofascore.js';
...
import { resolveLeagueReferee, formatRefereeDisplay } from './referees.js';
...
import { getCalibrationFactors } from './calibration.js';

const RECENT_FORM_WINDOW_DAYS = 120;
...
function enrichLeagueMatch(entry, competition, recentEntries = [], calibrationFactors = {}, sofaRefereeMap = new Map()) {
  ...
  const sofaReferee = getSofaRefereeForMatch(sofaRefereeMap, entry.dateIso, entry.homeName, entry.awayName);
  const referee = resolveLeagueReferee(entry.refereeName, sofaReferee);
...
export async function getLeagueTodayMatches(competitionId, dateIso = getDateISOInColombia(), force = false, { skipRecentForm = false } = {}) {
  ...
  const [raw, recentEntries, calibrationFactors, sofaRefereeMap] = await Promise.all([
    loadRawMatches(competition, dateIso, force),
    // skipRecentForm: para vistas masivas (p. ej. Inicio, que consulta ~100
    // competiciones a la vez) el escaneo de forma reciente es demasiado caro
    // multiplicado por cada competición. enrichLeagueMatch/attachLeagueRecentForm
    // ya manejan un recentEntries vacío con gracia (mismo camino que un equipo
    // en su primer partido), así que esto no rompe nada, solo omite el dato.
    skipRecentForm ? Promise.resolve([]) : getRecentFinishedEntries(competition).catch(() => []),
    getCalibrationFactors().catch(() => ({})),
    competition.sofaTournamentId && competition.sofaSeasonId
      ? loadSofaRefereesForCompetitionDate(dateIso, competition.sofaTournamentId, competition.sofaSeasonId, force).catch(() => new Map())
      : Promise.resolve(new Map()),
  ]);

  const enriched = raw
    .map((entry) => enrichLeagueMatch(entry, competition, recentEntries, calibrationFactors, sofaRefereeMap))
...
export function invalidateLeagueCaches(competitionId) {
  ...
  if (competition.sofaTournamentId && competition.sofaSeasonId) {
    invalidateSofaCompetitionRefereeCache(competition.sofaTournamentId, competition.sofaSeasonId);
  }
  if (competition.provider === 'sofascore') {
