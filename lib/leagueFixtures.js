...
function enrichLeagueMatch(entry, competition, recentEntries = [], calibrationFactors = {}) {
...
export async function getLeagueTodayMatches(competitionId, dateIso = getDateISOInColombia(), force = false) {
  ...
  const [raw, recentEntries, calibrationFactors] = await Promise.all([
  ...
  const enriched = raw
    .map((entry) => enrichLeagueMatch(entry, competition, recentEntries, calibrationFactors))
