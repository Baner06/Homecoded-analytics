import { attachRecentForm } from './teamForm.js';
...
import { invalidateLiveTrackerCache } from './liveTracker.js';
import { invalidateMatchStatsPanelCache } from './matchStatsPanel.js';
import {
...
async function fetchJson(url) {
  const res = await fetch(url, {
    headers: { Accept: 'application/json', 'User-Agent': 'Coded-Sports/1.0' },
    cache: 'no-store',
  });
...
function applyEspnResults(fixtures, espnByPair) {
  ...
  return fixtures.map((f) => {
    ...
    const oriented = orientLiveFeedToFixture(espn, f.homeTeam, f.awayTeam);
    const result = espnResultToScore(oriented);
    if (result) return { ...f, result, status: 'finished' };
    return f;
  });
}
...
function pickTeamsFromStatsFixture(statsRow, openFootball) {
  ...
  const byNumMatch = byNum.get(statsRow.matchNumber);
  if (byNumMatch) {
    const result = parseOpenFootballScore(byNumMatch);
    if (result) {
      return {
        homeTeam: normalizeOpenFootballTeamName(byNumMatch.team1),
        awayTeam: normalizeOpenFootballTeamName(byNumMatch.team2),
        result,
        source: 'openfootball-num',
      };
    }
  }
  ...
  if (statsRow.matchNumber >= 73) {
    ...
    if (byNumMatch) {
      return {
        homeTeam: normalizeOpenFootballTeamName(byNumMatch.team1),
        awayTeam: normalizeOpenFootballTeamName(byNumMatch.team2),
        result: parseOpenFootballScore(byNumMatch),
...
function backfillResultsFromOpenFootball(fixtures, openMatches) {
  const byPair = new Map();
  for (const m of openMatches) {
    ...
    const home = normalizeOpenFootballTeamName(m.team1);
    const away = normalizeOpenFootballTeamName(m.team2);
    byPair.set(teamPairKey(m.date, home, away), result);
  }

  return fixtures.map((f) => {
    if (f.result) return f;
    const result = byPair.get(teamPairKey(f.date, f.homeTeam, f.awayTeam));
    if (!result) return f;
...
async function buildAllFixtures({ forceSquadSync = false } = {}) {
  ...
  fixtures = backfillResultsFromOpenFootball(fixtures, openMatches);
...
export async function loadAllFixtures({ forceSquadSync = false } = {}) {
  const now = Date.now();
...
function resolveMatchResult(raw, liveFeed) {
  const espn = espnResultToScore(liveFeed);
  if (espn) return espn;
...
export function formatActualStats(actualStats) {
...
function enrichMatch(raw, liveScoresMap = new Map(), refereeMap = new Map(), sofaStats = null) {
  ...
  return {
    ...
    espnEventId: liveFeed?.espnEventId ? String(liveFeed.espnEventId) : null,
    analysis,
  };
}
...
export async function getTodayMatches(dateIso = getDateISOInColombia()) {
  ...
  const matches = attachRecentForm(
    todayRaw
      .map((raw) => enrichMatch(
        raw,
        liveScoresMap,
...
export async function getMatchById(matchId) {
  ...
  if (!id) return null;
  const all = await loadAllFixtures();
  const raw = all.find((f) => f.matchNumber === id);
  if (!raw) return null;

  const [liveScoresMap, refereeMap, sofaStatsMap] = await Promise.all([
    loadLiveScoresMap(raw.date),
    loadRefereesForDate(raw.date),
    loadSofaStatsForDate(raw.date),
  ]);

  return attachRecentForm([enrichMatch(
    raw,
    liveScoresMap,
    refereeMap,
    getSofaStatsForMatch(sofaStatsMap, raw.date, raw.homeTeam, raw.awayTeam)
...
export async function getFinishedMatches() {
  const all = await loadAllFixtures();
  const finishedRaw = all.filter((f) => f.status === 'finished' || f.result);
  const dates = [...new Set(finishedRaw.map((f) => f.date))];
  const enriched = await Promise.all(dates.map(async (date) => {
    const day = finishedRaw.filter((f) => f.date === date);
    const [liveScoresMap, refereeMap, sofaStatsMap] = await Promise.all([
      loadLiveScoresMap(date),
      loadRefereesForDate(date),
      loadSofaStatsForDate(date),
    ]);
    return day.map((raw) => enrichMatch(
      raw,
      liveScoresMap,
      refereeMap,
      getSofaStatsForMatch(sofaStatsMap, raw.date, raw.homeTeam, raw.awayTeam)
  ...
  return enriched.flat().filter((m) => m.status === 'finished');
}
