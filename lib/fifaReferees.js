...
function matchUtcDateIso(match) {
  if (!match?.Date) return null;
  return String(match.Date).slice(0, 10);
}

function matchColombiaDateIso(match) {
  if (!match?.Date) return null;
  return getDateISOInColombia(new Date(match.Date));
}
...
async function loadFifaMatches() {
  ...
  try {
    ...
    for (const match of data.Results || []) {
      ...
      const utcDate = matchUtcDateIso(match);
      const colombiaDate = matchColombiaDateIso(match);
      const home = fifaTeamName(match.Home);
      const away = fifaTeamName(match.Away);
      if (!colombiaDate || !home || !away) continue;
      ...
      byPairKey.set(teamPairKey(colombiaDate, home, away), payload);
      if (utcDate && utcDate !== colombiaDate) {
        byPairKey.set(teamPairKey(utcDate, home, away), payload);
      }
    }
  } catch (err) {
...
/** Mapa fecha|equipos → árbitro FIFA para un día (zona Colombia). */
export async function loadFifaRefereesForDate(dateIso) {
  const { byPairKey, byMatchNumber } = await loadFifaMatches();
  const map = new Map();

  for (const [key, entry] of byPairKey.entries()) {
    if (entry.colombiaDate === dateIso || entry.dateIso === dateIso) {
      map.set(key, entry);
    }
  }

  for (const [num, entry] of byMatchNumber.entries()) {
    if (entry.colombiaDate === dateIso || entry.dateIso === dateIso) {
      map.set(`m:${num}`, entry);
    }
  }
