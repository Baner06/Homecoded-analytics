...
/** Eliminatorias: prórroga + penales (~150 min desde el pitido inicial). */
export const KNOCKOUT_LIVE_WINDOW_MS = 150 * 60 * 1000;

export function isKnockoutStage(stage) {
  return stage && stage !== 'group-stage';
}

export function liveWindowMs(stage) {
  return isKnockoutStage(stage) ? KNOCKOUT_LIVE_WINDOW_MS : LIVE_MATCH_WINDOW_MS;
}
