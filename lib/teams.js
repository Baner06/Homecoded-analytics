import { getRosterShootersSync, filterEligibleShooters } from './squadRegistry.js';
...
const RAW = [
  ...
  { keys: ['Brazil', 'Brasil'], name: 'Brasil', flag: '🇧🇷', cornersFav: 6.5, cornersAgainst: 3.5, aggressiveness: 12.5, aerial: ['Marquinhos', 'Vinícius Júnior'] },
  ...
  { keys: ['Bosnia and Herzegovina', 'Bosnia y Herzegovina', 'Bosnia & Herzegovina'], name: 'Bosnia y Herzegovina', flag: '🇧🇦', cornersFav: 5.0, cornersAgainst: 4.5, aggressiveness: 14.0, aerial: ['Miralem Pjanić', 'Edin Džeko'] },
  ...
  { keys: ['Turkey', 'Türkiye', 'Turquía', 'Turkiye'], name: 'Turquía', flag: '🇹🇷', cornersFav: 5.5, cornersAgainst: 4.3, aggressiveness: 13.2, aerial: ['Çağlar Söyüncü', 'Hakan Çalhanoğlu'] },
];
...
export function resolveTeam(name) {
  ...
  const rosterShooters = getRosterShootersSync(base.name, base);
  const shooters = filterEligibleShooters(
    base.name,
    rosterShooters?.length ? rosterShooters : fallbackShooters
  );
...
export function listTeamsCatalog() {
  return RAW.map((t, index) => {
    const resolved = resolveTeam(t.keys[0]);
    return {
      id: index + 1,
      nombre: resolved.displayName,
      originalName: t.keys[0],
      iso: resolved.iso,
