...
function buildProbableActions(home, away, referee, ctx, calibrationFactors = {}) {
  ...
  const items = candidates.map(({ label, prob, eval: evalDef }) => ({
    label,
    probability: clampProb(prob * (calibrationFactors[evalDef.type] ?? 1)),
    eval: evalDef,
  }));
...
export function computePredictions(home, away, referee, calibrationFactors = {}) {
