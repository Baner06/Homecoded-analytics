...
export function evaluatePredictions(probableActions, actual) {
  ...
  for (const action of probableActions) {
    ...
    const entry = {
      ...
      evalType: action.eval?.type || null,
...
export function parseOpenFootballScore(match) {
  ...
  if (Array.isArray(match.score.pen) && match.score.pen[0] !== match.score.pen[1]) {
    out.decidedByPenalties = true;
    out.penHome = match.score.pen[0];
    out.penAway = match.score.pen[1];
  }
  if (Array.isArray(match.score.aet)) {
    out.aetHome = match.score.aet[0];
    out.aetAway = match.score.aet[1];
  }
  return out;
}

/** Ganador del partido (incluye penales vía ESPN u openfootball). */
export function pickMatchWinner(homeTeam, awayTeam, result, espnFeed = null) {
  ...
  if (result.penHome != null && result.penAway != null) {
    if (result.penHome > result.penAway) return homeTeam;
    if (result.penAway > result.penHome) return awayTeam;
  }
  if (espnFeed?.winnerTeam) {
    const w = espnFeed.winnerTeam.toLowerCase();
    if (homeTeam.toLowerCase() === w || resolveTeam(homeTeam).displayName.toLowerCase() === w) return homeTeam;
    if (awayTeam.toLowerCase() === w || resolveTeam(awayTeam).displayName.toLowerCase() === w) return awayTeam;
  }
  if (espnFeed?.homeWinner) return homeTeam;
  if (espnFeed?.awayWinner) return awayTeam;
  return null;
}

export function pickMatchLoser(homeTeam, awayTeam, result, espnFeed = null) {
  const winner = pickMatchWinner(homeTeam, awayTeam, result, espnFeed);
