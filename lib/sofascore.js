...
function eventDateIso(event) {
  ...
  return getDateISOInColombia(new Date(event.startTimestamp * 1000));
}
...
async function fetchTournamentEvents(direction, maxPages = 4, tournamentId = WC_TOURNAMENT_ID, seasonId = WC_SEASON_ID) {
  const events = [];
...
async function fetchAllTournamentEvents(tournamentId = WC_TOURNAMENT_ID, seasonId = WC_SEASON_ID) {
  const [last, next] = await Promise.all([
    fetchTournamentEvents('last', 4, tournamentId, seasonId),
    fetchTournamentEvents('next', 2, tournamentId, seasonId),
  ]);
  const byId = new Map();
...
const competitionRefereeCache = new Map(); // `${tournamentId}-${seasonId}|${dateIso}` -> { at, map }

/**
 * Árbitros SofaScore para una competición de clubes cualquiera (liga/copa fuera
 * del Mundial), identificada por su tournamentId/seasonId de Sofascore. Mismo
 * cruce fecha|equipos que loadSofaRefereesForDate, pero sin atarse al Mundial.
 */
export async function loadSofaRefereesForCompetitionDate(dateIso, tournamentId, seasonId, force = false) {
  if (!tournamentId || !seasonId) return new Map();

  const cacheKey = `${tournamentId}-${seasonId}|${dateIso}`;
  const now = Date.now();
  const cached = competitionRefereeCache.get(cacheKey);
  ...
  const map = new Map();
  try {
    const allEvents = await fetchAllTournamentEvents(tournamentId, seasonId);
    const dayEvents = allEvents.filter((ev) => eventDateIso(ev) === dateIso);
    ...
    console.error('[sofascore/competition-referees]', err.message);
  }

  competitionRefereeCache.set(cacheKey, { at: now, map });
  return map;
}

export function invalidateSofaCompetitionRefereeCache(tournamentId, seasonId) {
  if (!tournamentId) {
    competitionRefereeCache.clear();
    return;
  }
  const prefix = seasonId ? `${tournamentId}-${seasonId}|` : `${tournamentId}-`;
  [...competitionRefereeCache.keys()]
    .filter((k) => k.startsWith(prefix))
    .forEach((k) => competitionRefereeCache.delete(k));
}
