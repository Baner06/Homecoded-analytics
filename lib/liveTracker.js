const summaryUrl = (sportSlug = 'fifa.world') => `https://site.api.espn.com/apis/site/v2/sports/soccer/${sportSlug}/summary`;
const FETCH_HEADERS = {
...
export async function fetchLiveTracker(espnEventId, homeName, awayName, force = false, sportSlug = 'fifa.world') {
  const cacheKey = `${sportSlug}|${espnEventId}`;
  const now = Date.now();
  ...
  const res = await fetch(`${summaryUrl(sportSlug)}?event=${espnEventId}`, {
